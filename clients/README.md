...
`deviceId`/`teamId`는 클라이언트 생성 시 **기본값**으로 지정하면 매 호출에서 생략할 수 있습니다 (호출 시 넘기면 덮어씀). `eventId`·`ts`(타임스탬프)는 생략하면 **서버가 자동으로 채웁니다**.
...
SDK가 제공하는 동작은 5가지입니다. 각각이 대시보드에서 **무엇으로 보이는지**까지 설명합니다.
...
위 헬퍼로 표현 안 되는 이벤트를 직접 넣고 싶을 때 쓰는 원시 API입니다. `kind`·필수 필드를 직접 채운 객체/맵을 넘깁니다. 보통은 ①~③으로 충분합니다.
...
| 옵션 | 기본값 | 의미 |
|------|--------|------|
| `url` / `deviceId` / `teamId` | — | 수집 endpoint, 기본 디바이스/팀 |
